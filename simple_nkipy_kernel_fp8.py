...
def main():
    ...
    # Configuration
    size = 512  # Matrix size (1024x1024)
    warmup_iterations = 5
    ...
    output = np.zeros((size, size), dtype=ml_dtypes.float8_e5m2)
    ...
    # Compute throughput: matmul C = A^T @ B => 2 * M * K * N FLOPs
    flops = 2 * size * size * size
    mean_tflops = flops / (stats.mean_ms * 1e-3) / 1e12
    peak_tflops = flops / (stats.min_ms * 1e-3) / 1e12

    # Memory bandwidth: read A + B + write output (fp8 = 1 byte each)
    bytes_total = 3 * size * size * 1
    mean_bandwidth_gbs = bytes_total / (stats.mean_ms * 1e-3) / 1e9
    ...
    print(f"  Memory BW (mean):  {mean_bandwidth_gbs:.2f} GB/s")
    print("  ─────────────────────────────────────")
